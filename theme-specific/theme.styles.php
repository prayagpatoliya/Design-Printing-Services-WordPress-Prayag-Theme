...
if (!function_exists('alpha_color_customizer_get_css')) {

	function alpha_color_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = alpha_color_customizer_add_theme_fonts($fonts);
...
EOT;
			$rez = apply_filters('alpha_color_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
EOT;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(alpha_color_get_list_schemes()) : array($only_scheme);

			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = alpha_color_get_scheme_colors($scheme);
...
/*Elementor*/
.elementor-element.sc_price .elementor-column:nth-child(3n+3) .elementor-element .elementor-button-wrapper .elementor-button,
/* Tribe Events */
...
.wpb-js-composer .vc_tta-style-flat.vc_tta-shape-round .vc_tta-tabs-list a:hover,
.wpb-js-composer .vc_tta-style-flat.vc_tta-shape-round .vc_tta-tabs-list .vc_active a{
...
.scheme_self .elementor-dark-title .elementor-heading-title {
    color: {$colors['inverse_dark']};
}
...
.scheme_self.footer_wrap .sc_layouts_row_type_normal .sc_layouts_item a,
.scheme_self.footer_wrap .textwidget,
.scheme_self.footer_wrap .widget ul li a {
    color: {$colors['alter_text']};
}
.scheme_self.footer_wrap .sc_layouts_row_type_normal .sc_layouts_item a:hover,
...
.sc_recent_news .post_item .post_meta .post_counters a{
    background-color: transparent!important;
    color: {$colors['text']};
}
.sc_recent_news .post_item .post_meta .post_counters a:hover{
    color: {$colors['text_dark']};
}
...
.alpha_color_tabs .alpha_color_tabs_titles li a:hover {
	color: {$colors['text_dark']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.alpha_color_tabs .alpha_color_tabs_titles li.ui-state-active a {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_hover']};
}
...
.footer_default.scheme_dark .mejs-controls .mejs-button:hover,
.sidebar.widget_area .mejs-controls .mejs-button:hover{
	color: {$colors['extra_link']};
}
...
.sidebar.scheme_self .widget:nth-child(4n+1):before {
    border-top-color: {$colors['text_link2']};
}
.sidebar.scheme_self .widget:nth-child(4n+2),
.sidebar.scheme_self .widget:nth-child(4n+2):before {
    border-top-color: {$colors['text_link']};
}
.sidebar.scheme_self .widget:nth-child(4n+3):before {
    border-top-color: {$colors['text_hover']};
}
.sidebar.scheme_self .widget:nth-child(4n+4):before {
...
header .elementor-element .sc_layouts_menu_nav>li ul {
   background-color: {$colors['input_dark']};
}

/* Elementor Footer Links Colors */
footer .elementor-element .sc_layouts_menu li a span,
footer .elementor-element .sc_layouts_iconed_text span {
    color: {$colors['text_light']};
}
footer .elementor-element .sc_layouts_menu li a:hover span,
footer .elementor-element .sc_layouts_iconed_text a:hover span {
    color: {$colors['text_hover']};
}
...
.elementor-widget-container .socials_wrap .social_item:hover .social_icon{
    color: {$colors['text_dark']};
}
...
EOT;
					...
					$rez = apply_filters('alpha_color_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'alpha_color_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>